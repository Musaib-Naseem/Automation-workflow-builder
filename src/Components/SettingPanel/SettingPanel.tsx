...
const SettingPanel = React.memo(
  ({
    ...
    const setExecutionNode = useWorkflowStore(
      (state) => state.setExecutionNode,
    );
    ...
    const showDuplicateError = useWorkflowStore(
      (state) => state.showDuplicateError,
    );
            ...
            {selectedNode && (
              <div className="pl-8 pt-4">
                ...
                <div className="cursor-not-allowed flex items-center border border-[#D1D5DB] rounded-md w-[90%] mt-2 px-1 bg-[#F9FAFB] hover:shadow-sm transition">
                  <div
                    className={`w-8 h-8 flex items-center justify-center rounded-lg ${config?.bg}`}
                  >
