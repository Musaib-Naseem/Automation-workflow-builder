...
const Canvas = React.memo(
  ({ nodes, edges, setSelectedNode, isValidConnection }: CanvasProps) => {
    const setShowExecutionPanel = useWorkflowStore(
      (state) => state.setShowExecutionPanel,
    );
    const setEdges = useWorkflowStore((state) => state.setEdges);
    const setNodes = useWorkflowStore((state) => state.setNodes);
    const setNewUpdateNode = useWorkflowStore(
      (state) => state.setNewUpdateNode,
    );
    ...
    const onConnect = (connection: Connection) => {
      ...
      const graph = BuildGraph(UpdatedEdgeAll);
      ...
      const updatedNodes = nodes.map((node) => ({
        ...
        style: {
          ...node.style,
          backgroundColor: visited.has(node.id) ? "#DCFCE7" : "#FEE2E2",
          border: visited.has(node.id) ? "2px solid green" : "2px solid red",
        },
      }));
    ...
    const onEdgesDelete = (deletedEdges: nodeProps[]) => {
      let UpdatedEdgeAll = edges.filter(
        (edge) => !deletedEdges.some((del) => del.id == edge.id),
      );
      ...
      if (visited.size > 0) {
        toast.error(
          `Workflow has ${nodes.length - visited.size} disconnected nodes`,
        );
      }
      ...
      setEdges(
        edges.filter((edge) => !deletedEdges.some((del) => del.id == edge.id)),
      );
      ...
      const updatedNodes = nodes.map((node) => ({
        ...
        style: {
          ...
          ...(executionNodeId === node.id
            ...
            : {
                backgroundColor: visited.has(node.id) ? "#FEE2E2" : "#DCFCE7",
        ...
        {nodes.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none ml-[-100px]">
            <div className="text-center">
              ...
              <p className="mt-2 text-gray-500">Create your first node.</p>
            </div>
          </div>
        )}
      </div>
